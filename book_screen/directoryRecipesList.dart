...
class _DirectoryRecipesListState extends State<DirectoryRecipesList> {
  ...
  Future<bool> refresh() async {
    await Future.delayed(Duration(seconds: 1), getRecipes);
    return true;
  }
  ...
  Future<void> loadSavedRecipe() async {
    widget.directory.recipes = [];
    String uid;
    String recipeId;

    QuerySnapshot savedRecipes =
        await UserFromDB.getUserSavedRecipes(widget.uid);
    for (int i = 0; i < savedRecipes.documents.length; i++) {
      ...
      bool check = false;
      for (int i = 0; i < widget.directory.recipes.length; i++) {
        if (widget.directory.recipes[i].id == r.id) {
          check = true;
        }
      }
      if (!check) {
        if (mounted) {
          setState(() {
            widget.directory.recipes.add(r);
          });
        }
      }
    }
  ...
  void getDirectoryRecipes() async {
    List<Recipe> recipes = await RecipeFromDB.getDirectoryRecipesList(
        widget.uid, widget.directory.id);
    widget.directory.initRecipes(recipes);
  }
}
